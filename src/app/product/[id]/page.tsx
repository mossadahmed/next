  ...
  const renderStars = (rate: number) => {
    ...
    return (
      <>
        ...
        {[...Array(emptyStars)].map((_, i) => <FaRegStar key={`empty-${i}`} className="text-muted" />)}
      </>
    );
  };
      ...
      <Row className="g-5">
        <Col md={6} className="text-center">
          <Card className="shadow-sm p-3">
            ...
            />
            <Badge bg="info" className="mt-3 text-capitalize">{product.category}</Badge>
          </Card>
        </Col>
            ...
            <Col sm={6}>
              <Button
                ...
                onClick={() => navigator.share && navigator.share({
                  title: product.title,
                  text: typeof product.description === 'string' ? product.description : undefined,
                  url: `${product.id}`
                })}
              >
                <FaShareAlt /> Share
